...
condwhile:                                        ; preds = %loopwhile, %"Func entry"
  ...
  %fcmpgttmp = fcmp ogt float %term2, %eps3
  %whilecond = icmp ne i1 %fcmpgttmp, false
...
loopwhile:                                        ; preds = %condwhile
  ...
  %negflo = fneg float %alt16
  store float %negflo, ptr %alt, align 4
...
afterwhile:                                       ; preds = %condwhile
  ...
  %print_float = call float @print_float(float %cos19)
