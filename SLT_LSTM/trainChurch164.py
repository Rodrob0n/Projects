...
test = pd.read_csv("testss_church.csv")
...
'FIRST', 'WIND', 'COOK', 'BLUE', 'LOVE', 'YES', 'COLD', 'HELP', 'WATER', 'COOKIE', 'NEED', 'START', 'SHOES', 'BAD', 'OPEN', 'HOUSE', 'BOY', 'FISH', 'BEDROOM', 'NOW', 'TABLE', 'PEN', 'TREE', 'LONG', 'CHURCH',
...
#End 
#ChurchCheckpoint = ModelCheckpoint(bestModelName, monitor='val_accuracy',
#                            verbose=1, save_best_only=True, mode='max')

#lossChurchCheckpoint = ModelCheckpoint("LossChurchCheckpoint.h5", monitor='val_loss',
...
"""history = architecture.fit(
    ...
    callbacks=[ChurchCheckpoint, slowLR, lossChurchCheckpoint])
"""
...
minlossModel.compile(optimizer='adam',
                      loss=lossF,
                        metrics=['accuracy', TopKCategoricalAccuracy(k=3, name='top_3_accuracy')])
minlossModel.load_weights("LossChurchCheckpoint.h5")
...
# After training completes, add this code to visualize training progress
    ...
    plt.tight_layout()
    plt.savefig('Results/church/training_history_church.png')
    plt.close()
...
print(f"Best val_ac model test accuracy: {acc1*100:.2f}%")
...
print(f"Loss ChurchCheckpoint model test accuracy: {acc2*100:.2f}%")
print(f"Loss ChurchCheckpoint model loss: {loss2:.2f}")
...
import os
os.makedirs("Results\church", exist_ok=True)
...
def saveNewModelsEval(report):
    os.makedirs("Results/church", exist_ok=True)
    with open("Results/church/classification_report.txt", "w") as f:
    ...
    with open("Results/church/confPairs.txt", "w") as f:
    ...
    print("Confusion pairs saved to Results/church/confusion_pairs.txt")
    ...
    plt.savefig('Results/church/confusion_matrix.png', dpi=150)
    plt.close()

    print("Confusion matrix visualization saved to Results/church/confusion_matrix.png")
    print("Report saved to Results/church/classification_report.txt")



# Load and evaluate saved model
savedModelName = "Models/LastSize/BestChurch.h5"
...
try:
    ...
    if(not os.path.exists(savedModelName)):
        ...
        with open("BestChurchModelDescription.txt", "w") as file: 
        ...
        model2.load_weights(savedModelName)
...
# …existing code after you’ve generated y_true, y_pred…

# Inspect all PEN examples (PEN is in all_labels)
pen_idx = label_map['PEN']
video_files = test['Video file'].tolist()

print("\n--- Inspecting PEN (zero‐precision/recall) ---")

# 1) True PEN samples: what did the model predict?
print("True PEN samples (should be PEN):")
for i, true_lbl in enumerate(y_true):
    if true_lbl == pen_idx:
        print(f"  {video_files[i]} → predicted: {all_labels[y_pred[i]]}")

# 2) Predicted PEN samples: what were the true labels?
print("\nSamples predicted as PEN:")
for i, pred_lbl in enumerate(y_pred):
    if pred_lbl == pen_idx:
        print(f"  {video_files[i]} → true: {all_labels[y_true[i]]}")
